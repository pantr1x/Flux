...
export const TOOL_FILE = { python: 'a.py', node: 'a.js', java: 'a.java', cpp: 'a.cpp', go: 'a.go', csharp: 'a.cs', rust: 'a.rs', ruby: 'a.rb', php: 'a.php', lua: 'a.lua' };
...
export function createTools({ toast }) {
  ...
  function bind(root) {
    root.addEventListener('click', (e) => {
      const b = e.target.closest('[data-tc-install],[data-tc-web]');
      if (!b) return;
      e.stopPropagation();
      if (b.dataset.tcInstall) install(b.dataset.tcInstall);
  ...
  return { status, info, row, bind, install, ask, onChange, mb, canInstall: () => !!cache?.canInstall };
}
